...
const FlashcardManager = ({ documentId }) => {
  ...
  const fetchFlashcardSets = async () => {
    setLoading(true);
    try {
      const result = await flashcardService.getFlashcardForDocument(documentId);
      // result is { success, count, data }
      setFlashcardSets(result.data || []); // <-- use the array inside data
    } catch (error) {
  ...
  const handleToggleStar = async (cardId) => {
    try {
      ...
      toast.success("Flashcard starred status succesfully!");
    } catch (error) {
